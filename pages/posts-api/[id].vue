<template>
    <div>
        <Title>Post {{ $route.params.id }} | {{ title }}</Title>
        <p>Post {{ $route.params.id }} | {{ title }}</p>
        <h1>HelloPosts</h1>
        {{ $route }}
        {{ $route.params.id }}
                    ...
                    <div class="px-6">
                        ...
                        <div class="text-center mt-12">
                            ...
                            <div class="text-sm leading-normal mt-0 mb-2 text-gray-500 font-bold uppercase">
                                ...
                                {{ post.title }} - {{ post.height }}
                            </div>
                            <div class="mb-2 text-gray-700 mt-10">
                                <i class="fas fa-briefcase mr-2 text-lg text-gray-500"></i>{{ post.countries }}
                            </div>
                            <div class="mb-2 text-gray-700">
                                <i class="fas fa-university mr-2 text-lg text-gray-500"></i>{{ post.title }} - {{ post.height }}
                            </div>
                        </div>
...
const {data: post} = await useFetch(`https://api.nuxtjs.dev/posts/${route.params.id}`)
</script>
